...
Deterministic gathering may use capability hints such as correct-tool preparation and sneak-while-breaking, but `SafetyPolicy` and scoped `ResourceMutationPermit` remain authoritative. Gather quantities use minimum-fulfillment semantics: requesting 4 means “collect at least 4”; a safe chain may collect more without turning the goal into an error. Multi-block acceleration still fails closed unless the capability advertises a finite bounded `max_chain`, `same_block_only=true`, and an `exact_block` that matches the concrete candidate being mutated.

Resource Profiles separate source-block semantics from collected-item semantics. For example, `iron_ore` / `deepslate_iron_ore` are gathered as `raw_iron`. Silk Touch remains disallowed when it would change the expected collected item, while Fortune is allowed when the profile guarantees at least one acceptable drop per broken block; surplus drops satisfy the minimum earlier and remain valid inventory. Variable-drop ores such as copper, redstone, lapis, and nether gold use the same minimum-fulfillment accounting.

When MoxueBridge exposes `GET /api/v1/resources`, authoritative server resource descriptors override the built-in vanilla fallback profiles. The catalog can describe mod/plugin resource IDs, aliases, source blocks, collected items, minimum drop counts, tool kinds, forbidden enchantments, capability IDs, related leaves, cleanup policy, and confidence. Unknown or non-`minecraft:` resources are never given invented vanilla semantics when no authoritative catalog entry exists.

Ordinary resource discovery is visible-only; loaded-but-hidden blocks are rejected instead of being used like X-ray. `explore_resource` performs bounded no-dig exploration through safe visible waypoints and re-scans for visible resources after each move. Active goals may also be suspended by the deterministic hostile-mob supervisor, retreated to safety, and resumed with the original minimum gather target preserved.

Tree resources may declare `natural_decay`, `remove_after_felling`, or `preserve` leaf cleanup. `remove_after_felling` is bounded and only targets decaying/natural related leaves near the felled tree; it does not turn logs and leaves into one unconstrained VeinMiner group.
